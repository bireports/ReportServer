...
public class TsDiskUIServiceImpl implements TsDiskUIService {
	...
	@Inject
	public TsDiskUIServiceImpl(FormatUiHelper formatUiHelper,
	...
	@Override
	public <A extends AbstractTsDiskNodeDto> List<ColumnConfig<A, ?>> createGridColumnConfig(Class<A> type, boolean addIdColumn){
		...
		if(addIdColumn){
			ColumnConfig<A,A> idColumn = new ColumnConfig<A,A>(new IdentityValueProvider<A>("__id"), 90, TsFavoriteMessages.INSTANCE.idLabel());
		...
		/* name */
		ColumnConfig<A, String> nameColumn = new ColumnConfig<A, String>(new DisplayTitleValueProvider<A>(), 200, BaseMessages.INSTANCE.name());
		nameColumn.setMenuDisabled(true);
		
		columns.add(nameColumn);
		
		ColumnConfig<A, A> ccLastModified = new ColumnConfig<A,A>(new IdentityValueProvider<A>("__last"), 150, TsFavoriteMessages.INSTANCE.lastModified()); 
	...
	private Date getModelLastUpdated(AbstractTsDiskNodeDto model){
		if(model instanceof TsDiskGeneralReferenceDto){
			...
			if(null == refLastUpdated || (null != model.getLastUpdated() && refLastUpdated.before(model.getLastUpdated())))
