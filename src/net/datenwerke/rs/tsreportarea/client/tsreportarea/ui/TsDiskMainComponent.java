		...
		RpcProxy<AbstractNodeDto, List<AbstractNodeDto>> proxy = new RpcProxy<AbstractNodeDto, List<AbstractNodeDto>>() {
			...
			public void load(AbstractNodeDto loadConfig,
				...
				if(null == loadConfig)
					...
					treeLoaderDao.getChildren((AbstractNodeDto) loadConfig, false, callback);
			}
...
//		explorerToolbar.add(new FillToolItem());
		...
		/* expand all button */
		DwTextButton expandAllButton = new DwTextButton(BaseIcon.EXPAND);
		...
		/* collapse all button */
		DwTextButton collapseAllButton = new DwTextButton(BaseIcon.COLLAPSE);
	...
	public void folderOpened(TsDiskFolderDto folder, final Long selectChildId, final boolean execute) {
		...
		listPanel.setHeadingText(TsFavoriteMessages.INSTANCE.listPanelHeader(null == folder ? "Root" : folder.getName()));
	...
	public void importReport(ReportDto report, boolean copyReport, boolean reference) {
		...
		favoriteDao.importReport(currentSpace, currentFolder, report, copyReport, reference, new RsAsyncCallback<List<TsDiskReportReferenceDto>>(){
			@Override
