...
//		Connection con = dataSourceTransformer.transform(bReport, parameters);
	...
	protected CompiledReport executeReport(OutputStream os, Connection connection, JxlsReport bReport,
		...
		try{
			...
			/* get template */
			byte[] reportBytes = bReport.getReportFile().getContent();
			
			return outputGenerator.exportReport(os, reportBytes, connection, parameters, outputFormat, configs);
		}catch(Exception e){
