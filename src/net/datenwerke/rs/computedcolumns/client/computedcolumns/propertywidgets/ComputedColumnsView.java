...
public class ComputedColumnsView extends ReportExecutorMainPanelView {
	...
	protected void importColumn(final ComputedColumnDto toImport) {
		...
		for(final ComputedColumnDto col : store.getAll()){
			if( name.equals(col.getName()) ){
				/* if description and expression are the same, continue .. else ask if we should update */
				String expr = col.getExpression() != null ? col.getExpression() : "";
				String desc = col.getDescription() != null ? col.getDescription() : "";
				
				String importExpr = toImport.getExpression() != null ? toImport.getExpression() : "";
				String importDesc = toImport.getDescription() != null ? toImport.getDescription() : "";
				
				if(expr.equals(importExpr) && desc.equals(importDesc))
					continue;
				...
				cmb.addDialogHideHandler(new DialogHideHandler() {
					...
					public void onDialogHide(DialogHideEvent event) {
						if (event.getHideButton() == PredefinedButton.YES) {
							...
							store.add(cloneColumn(toImport));	
					}
				});
				cmb.show();
	...
	protected void displayEditColumnDialog(final ComputedColumnDto selectedColumn, final boolean removeOnCancel) {
		...
		dialog.setHeadingText(ComputedColumnsMessages.INSTANCE.editColumnHeading());
