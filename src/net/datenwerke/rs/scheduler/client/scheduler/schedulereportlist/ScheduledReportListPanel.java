...
public class ScheduledReportListPanel extends DwBorderContainer {
	...
	private final  boolean displayOwnerColumn;
	...
	public ScheduledReportListPanel(
		...
		@Assisted boolean displayOwnerColumn
		){
	...
	private void createDetailPanel() {
		...
		detailPanel.setHeadingText(SchedulerMessages.INSTANCE.detailPanelTitle());
	...
	private void createListPanel() {
		...
		listPanel.setHeadingText(SchedulerMessages.INSTANCE.listPanelHeading());
		...
		/* create grid */
		grid = new Grid<ReportScheduleJobListInformation>(listStore, schedulerService.createReportScheduleListColumnModel(displayJobColumn, displayOwnerColumn));
	...
	private void displayDetail(ReportScheduleJobListInformation selected, ReportScheduleJobInformation result) {
		...
		propertiesPanel.setHeadingText(SchedulerMessages.INSTANCE.propertiesLabel());
		...
		nextSchedulePanel.setHeadingText(SchedulerMessages.INSTANCE.scheduleListLabel());
		...
		iconColumn.setCell(new AbstractCell<ExecutionLogEntryDto>() {
			...
			public void render(com.google.gwt.cell.client.Cell.Context context,
				...
				if(model instanceof ExecutionLogEntryDto){
					...
					else if(OutcomeDto.VETO == entry.getOutcome())
	...
	private Grid<KeyValueProperty> getDetailGrid( final ReportScheduleJobListInformation selected, final ReportScheduleJobInformation result) {
		...
		if(null != result.getScheduleDefinition().getReport()){
			...
			store.add(new KeyValueProperty(SchedulerMessages.INSTANCE.scheduledBy(), selected.getOwnerName()));
			...
			store.add(new KeyValueProperty(SchedulerMessages.INSTANCE.jobExecutionStatus(), String.valueOf(result.getExecutionStatus())){
				...
				public void onRowDoubleClick(RowDoubleClickEvent event) {
					...
					if(! user.getId().equals(result.getOwnerId()) && ! securityService.hasRight(SchedulingAdminViewGenericTargetIdentifier.class, ExecuteDto.class))
						return;
	...
	public boolean isDisplayOwnerColumn() {
		return displayOwnerColumn;
	}
