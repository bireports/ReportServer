...
public class TableReportPreviewView extends AbstractReportPreviewView implements PageablePreviewView {
	...
	protected void updateColumnWidths() {
		((TableReportDtoDec)report).setClearColumnWidth();
		for(int i = 0; i < grid.getColumnModel().getColumnCount(); i++){
			int width = grid.getColumnModel().getColumnWidth(i);
	...
	protected int getColWidth(ColumnDtoDec column, TableReportDtoDec tReport) {
		if(tReport.isOptimalcolumnWidth())
			return computeOptimalColumnWidth(tReport);
	...
	protected void previewRow(int rowIndex, final PreviewModel model) {
		...
		window.setHeadingText(TableMessages.INSTANCE.rowPreviewHeader());
