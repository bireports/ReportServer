...
public class ColumnFilterWindow extends DwWindow {
	...
	public ColumnFilterWindow(
		...
		this(report, column, executeToken, 
			"enable".equals(((TableReportDtoDec)report).getReportProperty(BaseReportEngineUiModule.REPORT_PROPERTY_DL_FILTER_DEFAULT_CONSISTENCY, "enable")),  
			"true".equals(((TableReportDtoDec)report).getReportProperty(BaseReportEngineUiModule.REPORT_PROPERTY_DL_FILTER_SHOW_CONSISTENCY, "true")));
	}
	...
	private void initUi(TableReportDto report, final ColumnDto column, boolean forceConsistency, boolean showConsistency) {
		...
		setHeadingText(FilterMessages.INSTANCE.filterDialogHeading(column.getName(), null != column.getType() ? SqlTypes.getName(column.getType()) : ""));
		setBodyBorder(true);
