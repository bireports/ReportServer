...
//			ButtonGroup group = new ButtonGroup(ext.getColumns());
			...
			group.setHeadingText(ext.getLabel());
	...
	protected void addRepotExportFormat(final AddReportExportFormatProviderDto ext) {
		ReportExporterImpl exporter = new ReportExporterImpl() {
			...
			public boolean consumes(ReportDto report) {
				...
				if(null != ext.getReportIdWhitelist()){
					boolean found = false;
					for(Long id : ext.getReportIdWhitelist()){
				...
				if(null != ext.getParentIdWhitelist() && report instanceof ReportVariantDto){
					Long parentId = report.getParentNodeId();
					if(null == parentId)
						return false;
				...
				if(null != ext.getReportKeyWhitelist()){
					boolean found = false;
					for(String key : ext.getReportKeyWhitelist()){
				...
				if(null != ext.getParentKeyWhitelist() && report instanceof ReportVariantDto){
					String parentKey = report.getParentReportKey();
					if(null == parentKey)
						return false;
							
					boolean found = false;
					for(String key : ext.getParentKeyWhitelist()){
