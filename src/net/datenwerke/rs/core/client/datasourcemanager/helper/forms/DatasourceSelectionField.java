...
public class DatasourceSelectionField implements HasValueChangeHandlers<DatasourceContainerDto> {
	...
	private final boolean displayConfigFields;
	...
	private Iterable<Widget> datasourceConfigFields;
	...
	public DatasourceSelectionField(
		...
		@Assisted boolean displayConfigFields,
		...
		this.displayConfigFields = displayConfigFields;
	...
	protected void updateFieldsAndModel(DatasourceDefinitionDto datasource, final DatasourceContainerProviderDto datasourceContainerProvider) {
		...
		if(null != configContainer){
			...
			index = container.getWidgetIndex(configSet);
			configSet.removeFromParent();
		}
		datasourceConfigFields = null;
		
		/* display new specialized fields */
		datasourceConfigFields = datasourceConfigConfigurator.getFormfields(datasourceConfig, datasource, this);

		if(null != datasourceConfigFields && datasourceConfigFields.iterator().hasNext()){
//			FieldSet configSet = new FieldSet();
			SimpleContainer configSet = new SimpleContainer();
//			configSet.setHeadingText(DatasourcesMessages.INSTANCE.dataSourceConfigTitle());
			...
			if(datasourceConfigFields instanceof SimpleForm){
				configContainer.add((Widget)datasourceConfigFields);
			} else {
				for(Widget field : datasourceConfigFields){
