...
//		resp.setDateHeader("Expires", 1);	
		...
		if(supportsStreaming(referenceReport, backLinkSet, outputFormat, reportExecutorConfigs, req)){
			...
			exeucteReport(tos, req, referenceReport, backLinkSet, outputFormat, reportExecutorConfigs);
			
			storeInCache(rexToken, baseFilename, mimetype, extension, bos.toByteArray(), req, entry);
		} else {
			/* execte report */
			CompiledReport executedReport = exeucteReport(null, req, referenceReport, backLinkSet, outputFormat, reportExecutorConfigs);
	...
	protected CompiledReport exeucteReport(OutputStream os, HttpServletRequest req, Report report,
			ParameterSet backLinkSet, String outputFormat,
