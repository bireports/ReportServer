...
public class GroupForm extends FormView {
	...
	@Inject
	public GroupForm(
		@UserManagerTreeBasicSingleton UITree basicTreeForUserSelection,
		@UserManagerTreeBasicSingleton UITree basicTreeForGroupSelection
		){
		/* store objects */
	...
	private Widget getOuGrid(GroupDto group) {
		...
		/* create grid */
		ToolBarEnhancedDeletableRowsGridForTreeItemSelection wrapper = new ToolBarEnhancedDeletableRowsGridForTreeItemSelection(ouStore, new ColumnModel<GroupDto>(columns), basicTreeForGroupSelection, OrganisationalUnitDto.class);
		Grid<AbstractNodeDto> grid = wrapper.getGrid();
	...
	protected void doPreSubmit(AbstractNodeDto node) {
		...
		if(! (node instanceof GroupDto))
			throw new IllegalArgumentException("passed argument should be of type RoleDTO"); //$NON-NLS-1$
